import tsIndexPacker from "../core";

describe("tsIndexPackerCore", () => {
  it("generate ts", async () => {
    const result = await tsIndexPacker({
    ...
    expect(result).toEqual(
      [
        ...
        "const tsIndexPacker = {1:ASSETS_1,2:ASSETS_2,3:ASSETS_3}",
  ...
  it("generate ts with prefix", async () => {
    ...
    expect(result).toEqual(
      [
        ...
        "const tsIndexPacker = {1:TXT_1,2:TXT_2,3:TXT_3}",
        "export default tsIndexPacker;"
      ].join("\n")
    );
  });
